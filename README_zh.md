...
NexusMind 现在采用双通道检索流程，特别适合真实生产排障与 AI 咨询场景：

- 语义检索：理解概念相似问题
- BM25 风格词法检索：精确命中 `ERR_AUTH_401`、API 名、类名等关键词
- 倒数排名融合（RRF）：合并两路结果，提升稳定性
- 可观测调试接口（`query_debug`）：可追踪检索来源与排序依据

这让复杂技术问答和故障定位的回答质量更稳、更可解释。
